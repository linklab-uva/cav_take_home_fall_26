#!/bin/bash
# macOS wrapper script to run test_metrics.sh inside Docker container
# Usage: ./scripts/test_metrics_docker.sh [container_name]
...
echo "=== Docker Test Runner for macOS ==="
echo ""
...
if [ -z "$WORKSPACE_PATH" ]; then
    ...
    if [ -z "$WORKSPACE_PATH" ]; then
        # Ask user or use a default
        echo "Could not auto-detect workspace path."
        echo "Please ensure the repo is cloned in the container, or specify the path manually."
...
docker cp "${SCRIPT_DIR}/test_metrics.sh" "${CONTAINER_NAME}:/tmp/test_metrics.sh" 2>/dev/null || true
...
# Check if bag file already exists in container
